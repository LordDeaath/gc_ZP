...
new const skin_name[] = "Banana Joe"
new const skin_info[] = "[6% Damage]"
new const skin_models[][] = { "bananajoe_gc" }
...
public Fw_Damage(victim, inflictor, attacker, Float:damage,bits)
{
	...
	if(zp_class_human_get_current(attacker) != g_SkinID)
		return HAM_IGNORED;
	SetHamParamFloat(4, damage * 1.06)
	return HAM_IGNORED;
}
public BuyClass(id)
{
	new Mnu = menu_create("Do you want to buy this class?^n\r(Class: Banana Joe | Price 5,000 AP)", "class_m")
...
public class_m(id, menu, item)
{
	switch( item )
	{
		case 0:
		{
			if(zp_ammopacks_get(id) >= 5000)
			{
				zp_assign_class_id(id,9,1)
				zp_ammopacks_set(id, zp_ammopacks_get(id) - 5000)
...
public zp_fw_class_human_select_pre(id, classid)
{
	...
	if(!zp_return_class_id(id,9))
		zp_class_human_menu_text_add("\r[Buy: 5,000 AP]")
