...
public plugin_init()
{
	...
	register_cvar("hlxce_webpage", "https://hlstats.gamerclub.net/stats/hlstats.php", FCVAR_SPONLY|FCVAR_SERVER)
	g_hlx_block_commands = register_cvar("hlx_block_commands", "1")
	g_hlx_message_prefix = register_cvar("hlx_message_prefix", "[GC Stats]")
	...
	g_HLstatsX_MainMenu = menu_create("GC Stats - Main Menu", "mainmenu_handle")
	...
	g_HLstatsX_AutoMenu = menu_create("GC Stats - Auto-Ranking", "automenu_handle")
	...
	g_HLstatsX_EventsMenu = menu_create("GC Stats - Console Events", "eventsmenu_handle")
...
public plugin_end()
{
	menu_destroy(g_HLstatsX_AutoMenu)
	menu_destroy(g_HLstatsX_EventsMenu)
	menu_destroy(g_HLstatsX_MainMenu)
	ArrayDestroy(g_HLstatsX_ColorArray)
}
...
psay (client_id[], is_colored, client_message[192])
{
	...
	if (client > 0) {
		...
		if ((player_index > 0) && (!is_user_bot(player_index)) && (is_user_connected(player_index))) {
			...
			if ((player_team != color_team) && (player_team > -1)) {
				message_begin(MSG_ONE, g_msgTeamInfo, {0,0,0}, player_index)
				write_byte(player_index)
