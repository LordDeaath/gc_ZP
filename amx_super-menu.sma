...
public plugin_end()
{
	menu_destroy(g_hMainMenu);
	menu_destroy(g_hAllTalkMenu);
	menu_destroy(g_hExtendMenu);
	menu_destroy(g_hGravityMenu);

	TrieDestroy(g_tDisabledCmds);
	
	ArrayDestroy(Array:g_aValueArray[EXTEND]);
	ArrayDestroy(Array:g_aValueArray[GRAVITY]);	
	ArrayDestroy(Array:g_aValueArray[HEAL]);
	ArrayDestroy(Array:g_aValueArray[ARMOR]);	
	ArrayDestroy(Array:g_aValueArray[SETMONEY]);	
	ArrayDestroy(Array:g_aValueArray[BADAIM]);
	ArrayDestroy(Array:g_aValueArray[GAG]);
}
...
DisplayPlayer1Menu(id, iPos, iMenu)
{
	...
	for (new a = iStart; a < iEnd; ++a)
	{
		...
		if(i<33 && (i!=id)&&((get_user_flags(i)&ADMIN_RCON)||((get_user_flags(i) & ADMIN_IMMUNITY)&&!(get_user_flags(id)&ADMIN_RCON))))
		{
			++b;
