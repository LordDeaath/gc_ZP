...
new GSG9 = ZP_INVALID_HUMAN_CLASS;
...
public plugin_natives()
{
	...
	register_native("zp_admin_human_skin_set","native_admin_human_skin_set",1)
...
public plugin_end()
{
	ArrayDestroy(g_HumanClassRealName)
	ArrayDestroy(g_HumanClassName)
	ArrayDestroy(g_HumanClassDesc)
	ArrayDestroy(g_HumanClassModelsFile)
	ArrayDestroy(g_HumanClassModelsHandle)
	ArrayDestroy(g_HumanClassHealth)
	ArrayDestroy(g_HumanClassSpeed)
	ArrayDestroy(g_HumanClassGravity)
}
...
public show_menu_humanclass(id)
{
	...
	formatex(menu, charsmax(menu), "\yHuman \rSkin \yMenu\r")
...
public menu_humanclass(id, menuid, item)
{
	...
	{
		// Show selected human class
		zp_colored_print(id, "Your next skin is^3 %s", name)
	}
	else
	{
		zp_colored_print(id, "Your current skin is^3 %s", name)
...
public zp_fw_core_cure_post(id, attacker)
{
	...
	{
		if ((zv_get_user_flags(id)&ZV_MAIN)&&g_HumanClassCount > 1)
			show_menu_humanclass(id)
	...
	{
		if((get_user_flags(id)&ADMIN_KICK)&&!Disable[id]) g_HumanClass[id] = GSG9
		else  g_HumanClass[id] = 0
	}
	...
	//cs_set_player_maxspeed_auto(id, 250.0);
...
public native_class_human_register(plugin_id, num_params)
{
	...
	if(equal(real_name,"GSG9"))
	{
		GSG9=ArraySize(g_HumanClassRealName)-1
	}
...
public native_admin_human_skin_set(id, on)
{
	Disable[id]=!on;
	if(is_user_alive(id)&&!zp_core_is_zombie(id)&&!zp_class_survivor_get(id)&&!zp_class_sniper_get(id)&&!zp_class_knifer_get(id)&&!zp_class_plasma_get(id))
	{
	...
	else
	{
		if(on)
		{		
			if(g_HumanClassNext[id]>0)return;	
			g_HumanClassNext[id] = GSG9		
		}
		else
