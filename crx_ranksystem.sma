...
public Cmd_GiveXP(id, iLevel, iCid)
{
	...
	new szPlayer[MAX_NAME_LENGTH], iPlayer
	read_argv(1, szPlayer, charsmax(szPlayer))

	if(equal(szPlayer, "STEAM_", 6) || equal(szPlayer, "VALVE_", 6))
	{
		iPlayer = find_player("c", szPlayer);
		if(!iPlayer)
		{
			new szAmount[CRXRANKS_MAX_XP_LENGTH]
			read_argv(2, szAmount, charsmax(szAmount))

			new iXP = str_to_num(szAmount)
			...
			{				
				case '+':
				{		
					formatex(szQuery, charsmax(szQuery), "UPDATE %s SET XP =  XP + '%d' WHERE Player = '%s'", g_eSettings[SQL_TABLE], iXP, szPlayer);
					SQL_ThreadQuery(g_iSqlTuple, "QueryHandler", szQuery)
					console_print(id, "%s's was given %d XP", szPlayer, iXP);
				}
				case '-':
				{			
					formatex(szQuery, charsmax(szQuery), "UPDATE %s SET XP =  XP - '%d' WHERE Player = '%s'", g_eSettings[SQL_TABLE], -iXP, szPlayer);
					SQL_ThreadQuery(g_iSqlTuple, "QueryHandler", szQuery)
					console_print(id, "%s's got %d XP removed", szPlayer, -iXP);
				}
				default:
				{					
					formatex(szQuery, charsmax(szQuery), "UPDATE %s SET XP = '%d' WHERE Player = '%s'", g_eSettings[SQL_TABLE], iXP, szPlayer);
					SQL_ThreadQuery(g_iSqlTuple, "QueryHandler", szQuery)
					console_print(id, "%s's Current XP: %d", szPlayer, iXP);	
				}
			}			
			return PLUGIN_HANDLED;
		}
	...
	switch(iType)
	{
		case SL_SAVE_DATA:
		{
			if(g_eSettings[USE_MYSQL])
			{
				...
				new szQuery[MAX_QUERY_LENGTH], szPlayer[MAX_SQL_PLAYER_LENGTH], szRank[MAX_SQL_RANK_LENGTH], szNextRank[MAX_SQL_RANK_LENGTH], szName[64]
				...
				formatex(szQuery, charsmax(szQuery), "REPLACE INTO `%s` (`Nickname`, `Player`, `XP`, `Level`, `Next XP`, `Rank`, `Next Rank`) VALUES ('%s', '%s', '%i', '%i', '%i', '%s', '%s');",\
				g_eSettings[SQL_TABLE], szName, szPlayer, g_ePlayerData[id][XP], g_ePlayerData[id][Level], g_ePlayerData[id][NextXP], szRank, szNextRank)
				SQL_ThreadQuery(g_iSqlTuple, "QueryHandler", szQuery)
				
				xp_log("%d saved for %s %s",g_ePlayerData[id][XP],szInfo,szName);	
			}
			else
			{
...
public QueryLoadData(iFailState, Handle:iQuery, szError[], iErrorCode, iData[], iSize)
{
	...
	xp_log("%d loaded for %s %s",g_ePlayerData[id][XP],g_ePlayerData[id][SaveInfo],name);		
	check_level(id, false)
...
check_level(const id, const bool:bNotify)
{
	...
	if(iLevel != g_ePlayerData[id][Level])
	{
		...
		if(bNotify && g_eSettings[LEVELUP_MESSAGE_TYPE]&&g_ePlayerData[id][Level]>1)
		{
			new szName[MAX_NAME_LENGTH]
			get_user_name(id, szName, charsmax(szName))
			...
			if(TrieKeyExists(g_tAP, szLevel))
			{
				...
				zp_ammopacks_set(id,zp_ammopacks_get(id)+iAP);
