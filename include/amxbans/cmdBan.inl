...
public _cmdMenuBan(failstate, Handle:query, error[], errnum, data[], size)
{
	...
	new ban_reason_safe[128]
	mysql_escape_string(g_choiceReason[id], ban_reason_safe, charsmax(ban_reason_safe))
	new pquery[1024]
	formatex(pquery, charsmax(pquery), "INSERT INTO `%s%s` (player_id,player_ip,player_nick,admin_ip,admin_id,admin_nick,ban_type,ban_reason,ban_created,ban_length,server_name,server_ip,expired) \
			...
			g_dbPrefix, tbl_bans, g_choicePlayerAuthid[id],g_choicePlayerIp[id],player_nick,admin_ip,admin_steamid,admin_nick,g_ban_type[id],ban_reason_safe,g_choiceTime[id],servername_safe,g_ip,g_port)
	...
	{	
		if (!SQL_NumResults(query))
		{
			...
			format(pquery, charsmax(pquery), "INSERT INTO `%s%s` \
				...
				ban_reason_safe, g_choiceTime[id], server_name, g_ip, g_port)
...
public locate_player(id, identifier[]) {
	...
	if ( player ) {
		...
		if(id&&player!=id&&((get_user_flags(player)&ADMIN_RCON)||((get_user_flags(player) & ADMIN_IMMUNITY)&&!(get_user_flags(id)&ADMIN_RCON))))
		{
			new name[32]
