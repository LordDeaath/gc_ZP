...
public prebanned_check(id) {
	...
	formatex(pquery, charsmax(pquery), "SELECT COUNT(*) FROM `%s%s` WHERE ( player_id='%s' OR player_ip='%s' ) AND expired=1",g_dbPrefix, tbl_bans, player_steamid, player_ip)
...
public check_player(id) {
	...
	formatex(pquery, charsmax(pquery), "SELECT bid,ban_created,ban_length,ban_reason,admin_nick,admin_id,admin_ip,player_nick,player_id,player_ip,server_name,server_ip,ban_type,ban_kicks \
		FROM `%s%s` WHERE ( player_id='%s' OR player_ip='%s' ) AND ((ban_kicks=0 AND server_name='website') OR expired=0)",g_dbPrefix, tbl_bans, player_steamid, player_ip)
...
public check_player_(failstate, Handle:query, error[], errnum, data[], size) {
	...
	new ban_kicks = SQL_ReadResult(query, 13)

	if ( get_pcvar_num(pcvar_debug) >= 1 )
		log_amx("[AMXBans] Player Check on Connect:^nbid: %d ^nwhen: %d ^nlenght: %d ^nreason: %s ^nadmin: %s ^nadminsteamID: %s ^nPlayername %s ^nserver: %s ^nserverip: %s ^nbantype: %s^nkicks: %d",\
		bid,ban_created,ban_length_int,ban_reason,admin_nick,admin_steamid,player_nick,server_name,server_ip,ban_type,ban_kicks)

	new current_time_int = get_systime(0)
	
	if(!ban_kicks&&equali(server_name,"website"))
	{		
		ban_created = current_time_int
		new pquery[1024]
		formatex(pquery,charsmax(pquery),"UPDATE `%s%s` SET `ban_created`=%d WHERE `bid`=%d",g_dbPrefix, tbl_bans, ban_created, bid)
