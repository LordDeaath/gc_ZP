...
new cvar_plasma_weapon_block
new cvar_knifer_weapon_block
new g_MsgAmmoPickup

new FrostPurchases[33],NapalmPurchases[33];
...
public plugin_end()
{
	ArrayDestroy(g_weapon_names)
	ArrayDestroy(g_weapon_items)
	ArrayDestroy(g_weapon_itemid)
}
...
public plugin_cfg()
{
	...
	cvar_plasma_weapon_block = get_cvar_pointer("zp_plasma_weapon_block")
	cvar_knifer_weapon_block = get_cvar_pointer("zp_knifer_weapon_block")
}

public plugin_natives()
{	
	register_native("zp_grenades_set_cost","native_grenades_set_cost")
	set_module_filter("module_filter")
	set_native_filter("native_filter")
}

public native_grenades_set_cost(plugin,params)
{
	// Is this our item?
	new index
	for (index = 0; index < ArraySize(g_weapon_items); index++)
		if (get_param(3) == ArrayGetCell(g_weapon_itemid, index)) break;
	...
	{
		set_param_byref(2, floatround((1.0+float((FrostPurchases[get_param(1)]<4?FrostPurchases[get_param(1)]:4))/2) * float(get_param_byref(2))))
	}		
}
...
public zp_fw_items_select_pre(id, itemid, ignorecost)
{
	...
	// Weapons available to sniper?
		...
		return ZP_ITEM_DONT_SHOW;
                ...
                // Weapons available to knifer?
	else if (LibraryExists(LIBRARY_KNIFER, LibType_Library) && zp_class_knifer_get(id) && get_pcvar_num(cvar_knifer_weapon_block))
		return ZP_ITEM_DONT_SHOW;

                // Weapons available to plasma?
	else if (LibraryExists(LIBRARY_PLASMA, LibType_Library) && zp_class_plasma_get(id) && get_pcvar_num(cvar_plasma_weapon_block))
		return ZP_ITEM_DONT_SHOW;
	...
	if(zp_class_survivor_get(id)&&(weaponid!=CSW_G3SG1&&weaponid!=CSW_SG550))
		return ZP_ITEM_DONT_SHOW;

	if (user_has_weapon(id, weaponid)&&weaponid!=CSW_HEGRENADE&&weaponid!=CSW_FLASHBANG)
...
public zp_fw_items_select_post(id, itemid, ignorecost)
{
	...
	else 
	{
		...
		else if(weaponid==CSW_FLASHBANG)
		{
			FrostPurchases[id]++
		}
