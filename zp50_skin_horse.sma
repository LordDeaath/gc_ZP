...
new const skin_name[] = "Horse Face"
new const skin_info[] = "[3% Damage]"
new const skin_models[][] = { "ahorse_gc" }
...
public Fw_Damage(victim, inflictor, attacker, Float:damage,bits)
{
	...
	if(zp_class_human_get_current(attacker) != g_SkinID)
		return HAM_IGNORED;
	SetHamParamFloat(4, damage * 1.03)
	return HAM_IGNORED;
}
public BuyClass(id)
{
	new Mnu = menu_create("Do you want to buy this class?^n\r(Class: Horse Face | Price 2,500 AP)", "class_m")
...
public class_m(id, menu, item)
{
	switch( item )
	{
		case 0:
		{
			if(zp_ammopacks_get(id) >= 2500)
			{
				zp_assign_class_id(id,7,1)
				zp_ammopacks_set(id, zp_ammopacks_get(id) - 2500)
...
public zp_fw_class_human_select_pre(id, classid)
{
	...
	if(!zp_return_class_id(id,7))
		zp_class_human_menu_text_add("\r[Buy: 2,500 AP]")
