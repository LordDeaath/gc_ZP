...
// =========== Main Config
#define CSW_BOW CSW_P90
#define weapon_bow "weapon_p90"
#define BOW_AMMOID 4
...
#define TIME_RELOADA 0.65
#define TIME_RELOADB 2.0
#define TIME_CHARGE 0.75

#define WEAPON_SECRETCODE 3102013 // Create Date
#define WEAPON_ANIMEXT "carbine"
#define WEAPON_OLDWMODEL "models/w_p90.mdl"
...
new g_SprId_LaserBeam, cvar_dm[2]
...
public plugin_init()
{
	...
	cvar_dm[0] = register_cvar("zp_dm_a","2.5")
	cvar_dm[1] = register_cvar("zp_dm_b","5.0")
	...
	//register_clcmd("admin_bow", "Get_CompoundBow")
...
public plugin_natives()
	register_native("buy_cbow", "Get_CompoundBow", 1)
...
public fw_Touch_Arrow(Ent, Id)
{
	...
	{ // Player
		...
		do_attack(Attacker, Id, 0, get_pcvar_float(cvar_dm[0]) * 1.5)
		engfunc(EngFunc_RemoveEntity, Ent)
	}
}
...
public ChargedDamage(id, Float:Start[3], Float:End[3])
{
	...
	{
		do_attack(id, pHit1, 0, get_pcvar_float(cvar_dm[1]))
	...
	{
		do_attack(id, pHit2, 0, get_pcvar_float(cvar_dm[0]))
	...
	{
		do_attack(id, pHit3, 0, get_pcvar_float(cvar_dm[0]))
	...
	if(is_user_alive(pHit4)) do_attack(id, pHit4, 0, get_pcvar_float(cvar_dm[0]))
...
{
	if(zp_item_zombie_madness_get(Victim) || zp_grenade_frost_get(Victim))
		return
