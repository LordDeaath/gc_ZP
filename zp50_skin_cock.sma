...
new const skin_name[] = "Cock"
new const skin_info[] = "[9% Damage]"
new const skin_models[][] = { "cock_gc" }
...
public Fw_Damage(victim, inflictor, attacker, Float:damage,bits)
{
	...
	if(zp_class_human_get_current(attacker) != g_SkinID)
		return HAM_IGNORED;
	SetHamParamFloat(4, damage * 1.09)
	return HAM_IGNORED;
}
public BuyClass(id)
{
	new Mnu = menu_create("Do you want to buy this class?^n\r(Class: Cock | Price 7,500 AP)", "class_m")
...
public class_m(id, menu, item)
{
	switch( item )
	{
		case 0:
		{
			if(zp_ammopacks_get(id) >= 7500)
			{
				zp_assign_class_id(id,10,1)
				zp_ammopacks_set(id, zp_ammopacks_get(id) - 7500)
...
public zp_fw_class_human_select_pre(id, classid)
{
	...
	if(!zp_return_class_id(id,10))
		zp_class_human_menu_text_add("\r[Buy: 7,500 AP]")
