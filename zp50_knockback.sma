...
//native zp_climb_zombie_is_climbing(id);
...
new cvar_knockback_nemesis,cvar_knockback_dione, cvar_knockback_assassin, cvar_knockback_dragon, cvar_knockback_nightcrawler, cvar_knockback_predator

//new cvar_knockback_climb;
//new g_iClimbZombieId;
...
public plugin_init()
{
	...
	//cvar_knockback_climb = register_cvar("zp_knockback_climb", "1.25");
	
	// Nemesis Class loaded?
	if (LibraryExists(LIBRARY_NEMESIS, LibType_Library))
		cvar_knockback_nemesis = register_cvar("zp_knockback_nemesis", "0")

	// Nemesis Class loaded?
	if (LibraryExists(LIBRARY_DIONE, LibType_Library))
		cvar_knockback_dione = register_cvar("zp_knockback_dione", "0")

	// Dragon Class loaded?
	if (LibraryExists(LIBRARY_DRAGON, LibType_Library))
		cvar_knockback_dragon = register_cvar("zp_knockback_dragon", "0")

	// Assassin Class loaded?
	if (LibraryExists(LIBRARY_ASSASSIN, LibType_Library))
		cvar_knockback_assassin = register_cvar("zp_knockback_assassin", "0")

	// Nightcrawler Class loaded?
	if (LibraryExists(LIBRARY_NIGHTCRAWLER, LibType_Library))
		cvar_knockback_nightcrawler = register_cvar("zp_knockback_nightcrawler", "0")		
	
	// Predator Class loaded?
	if (LibraryExists(LIBRARY_PREDATOR, LibType_Library))
		cvar_knockback_predator = register_cvar("zp_knockback_predator", "0")
}
...
public plugin_cfg()
{
    g_iClimbZombieId = zp_class_zombie_get_id("Climb Zombie");
}*/
...
{
	if (equal(module, LIBRARY_NEMESIS) ||equal(module, LIBRARY_DIONE) || equal(module, LIBRARY_DRAGON) || equal(module, LIBRARY_NIGHTCRAWLER) || equal(module, LIBRARY_PREDATOR) || equal(module, LIBRARY_ASSASSIN))
...
public fw_TraceAttack_Post(victim, attacker, Float:damage, Float:direction[3], tracehandle, damage_type)
{
	...
	if (LibraryExists(LIBRARY_DRAGON, LibType_Library) && zp_class_dragon_get(victim) && get_pcvar_float(cvar_knockback_dragon) == 0.0)
		return;

	// Nightcrawler knockback disabled, nothing else to do here
	if (LibraryExists(LIBRARY_NIGHTCRAWLER, LibType_Library) && zp_class_nightcrawler_get(victim) && get_pcvar_float(cvar_knockback_nightcrawler) == 0.0)
		return;
		
	// Predator knockback disabled, nothing else to do here
	if (LibraryExists(LIBRARY_PREDATOR, LibType_Library) && zp_class_predator_get(victim) && get_pcvar_float(cvar_knockback_predator) == 0.0)
		return;
	...
	else // Dione Class loaded?
	if (LibraryExists(LIBRARY_DIONE, LibType_Library) && zp_class_dione_get(victim))
	{
		// Apply dione knockback multiplier
		xs_vec_mul_scalar(direction, get_pcvar_float(cvar_knockback_dione), direction)
	}
	// Dragon Class loaded?
	else if (LibraryExists(LIBRARY_DRAGON, LibType_Library) && zp_class_dragon_get(victim))
	{
		// Apply dragon knockback multiplier
		xs_vec_mul_scalar(direction, get_pcvar_float(cvar_knockback_dragon), direction)
	}
	// Assassin Class loaded?
	...
	{
		// Apply dragon knockback multiplier
		xs_vec_mul_scalar(direction, get_pcvar_float(cvar_knockback_nightcrawler), direction)
	}
	// Predator Class loaded?
	else if (LibraryExists(LIBRARY_PREDATOR, LibType_Library) && zp_class_predator_get(victim))
	{
		// Apply dragon knockback multiplier
		xs_vec_mul_scalar(direction, get_pcvar_float(cvar_knockback_predator), direction)
	}
	/*else if (g_iClimbZombieId != -1 && g_iClimbZombieId == zp_class_zombie_get_current(victim) && zp_climb_zombie_is_climbing(victim))
        {
            xs_vec_mul_scalar(direction, get_pcvar_float(cvar_knockback_climb), direction);
        }*/
