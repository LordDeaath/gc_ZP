...
public cmdSayAdmin(id)
{
	...
	{
		ColorChat(id,GREEN,"[GC]^3 You are gagged, stfu")
		return PLUGIN_CONTINUE
	}
	...
	for (new bool:is_sender_admin = is_user_admin(id) != 0, i = 0; i < inum; ++i)
	{
		...
		{
			ColorChat(pl, GREEN, "(%l)^1 %s :^4  %s", is_sender_admin ? "ADMIN" : "PLAYER", name, message[1])
		}
	}
...
public cmdChat(id, level, cid)
{
	...
	format(message, 191, "(ADMINS)^1 %s :^4   %s", name, message)
...
public cmdSay(id, level, cid)
{
	...
	get_user_name(id, name, charsmax(name))
	userid = get_user_userid(id)
	ColorChat(0, GREEN, "(ALL)^1 %s :^4   %s",name, message);
...
public cmdPsay(id, level, cid)
{
	...
	if (id && id != priv)
		ColorChat(id, GREEN, "(%s)^1 %s :^4   %s", name, name2, message[length])
	
	ColorChat(priv, GREEN, "(%s)^1 %s :^4   %s", name, name2, message[length])
