...
new gMode_Predator, gMode_Dragon, gMode_Nightcrawler, gMode_Plague, /*gMode_Knifer,*/gMode_Ninja, gMode_SvA, g_NormalID, g_MultiID,MnF, Hunter;
...
public plugin_cfg()
{
	...
	Hunter = zp_gamemodes_get_id("Hunter Mode")
...
{
	if(equal(authid,"STEAM_0:0:844651")||equal(authid,"STEAM_0:1:103694247"))
	{
		HasAccess[id]=true;
	}
	else
...
public Pre_AdminSelectMode(id)
{
	...
	if(gNextRoundShouldBe<3)
	{
		...
		{					
			ColorChat(id, GREEN, "[GC]^1 You must wait^3 3 Rounds^1 between specials!")
			client_print(id,print_console, "You must wait 3 rounds between specials!")
			return PLUGIN_HANDLED;
		}
	}
...
public AdminSelectMode(id)
{
	...
	else menu_additem(Menu, "\dAlien Mode (No Acess)")
	
	menu_additem(Menu, "Monster Fight")
	menu_additem(Menu, "Plague Mode")
	menu_additem(Menu, "Terminal Infection")
	menu_additem(Menu, "Hunter Mode")
	#endif
	/*if(!gSniper)
...
public FormatPreText(id)
{
	...
	log_to_file("zp_mod_votes.log","Admin %s - started %d",Nick,gSelectedMode)
}
...
public VoteMenu(id)
{
	...
	if(gSelectedMode == gMode_Nemesis || gSelectedMode == Hunter || gSelectedMode == gMode_Dragon || gSelectedMode == gMode_Nightcrawler || gSelectedMode == gMode_Predator || gSelectedMode == gMode_Survivor || gSelectedMode == gMode_Plasma|| gSelectedMode == gMode_Ninja)//|| gSelectedMode == gMode_Winos ||gSelectedMode == gMode_Dione )
	{
...
public voteCount(id, key)
{
	...
	{		
		...
		case 2:
		{
			if(gSelectedMode == Hunter || gSelectedMode == gMode_Nemesis || gSelectedMode == gMode_Dragon || gSelectedMode == gMode_Nightcrawler || gSelectedMode == gMode_Predator || gSelectedMode == gMode_Survivor || gSelectedMode == gMode_Plasma|| gSelectedMode == gMode_Ninja)//|| gSelectedMode == gMode_Winos ||gSelectedMode == gMode_Dione )
			{
				get_user_authid(id, AuthID, charsmax(AuthID))
				if( !TrieKeyExists( g_tAuthIdBlackList, AuthID ) )
				{
...
public CountDown()
{
	...
	{
		iCount--
		if(gSelectedMode == gMode_Swarm)
			formatex(ModeName, charsmax(ModeName), "Terminal Infection")
		else 
			zp_gamemodes_get_name(gSelectedMode, ModeName, charsmax(ModeName))
...
{
	if(gSelectedMode == Hunter || gSelectedMode == gMode_Ninja ||gSelectedMode == gMode_Nemesis  || gSelectedMode == gMode_Dragon || gSelectedMode == gMode_Nightcrawler || gSelectedMode == gMode_Predator || gSelectedMode == gMode_Survivor || gSelectedMode == gMode_Plasma)//||gSelectedMode == gMode_Dione||gSelectedMode== gMode_Winos
	{				
...
{	
	...
	else 
	{
		...
		{
			new iRandom = random(20)	
			switch(iRandom)
			{
				case 0..5: gSelectedMode=g_NormalID
				case 6,9,12,15:
				{
					if(GetAliveCount() > 6)
						gSelectedMode=gMode_Swarm
					else gSelectedMode=g_NormalID
				}
				case 7,10,13,16:
				{
					if(GetAliveCount() > 9)
				...
				case 8,14: gSelectedMode=g_MultiID
				case 17..20: gSelectedMode=g_NormalID
			}		
			if(gSelectedMode == gMode_Swarm)
				formatex(ModeName, charsmax(ModeName), "Terminal Infection")
			else
