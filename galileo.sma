...
#define IS_NOMINATION_MID_PLAYERS_CONTROL_ENABLED() \
( get_real_players_number() < get_pcvar_num( cvar_voteMidPlayers ) \
&& get_pcvar_num( cvar_nomMinPlayersControl ) )
//
...
new cvar_recentMapsBannedNumberMin;
new cvar_recentMapsBannedNumberMid;
...
new Trie:g_midPlayerFillerMapGroupTrie;
...
public plugin_init()
{
	...
	cvar_recentMapsBannedNumberMin    = register_cvar( "gal_banrecent_min"                , "0"    );
	cvar_recentMapsBannedNumberMid    = register_cvar( "gal_banrecent_mid"                , "0"    );
		...
		while( !feof( recentMapsFileDescriptor ) )
		{
			...
			&& IS_MAP_VALID( recentMapName ) )
			{
				...
				if( !TrieKeyExists( g_recentMapsTrie, recentMapName ) )
				{
					...
					if(loadedMapsCount<get_pcvar_num(cvar_recentMapsBannedNumberMin)&&get_pcvar_num(cvar_recentMapsBannedNumberMin))
					{
						TrieSetCell( g_recentMapsTrie, recentMapName, 2 );
					}
					else if(loadedMapsCount<get_pcvar_num(cvar_recentMapsBannedNumberMid)&&get_pcvar_num(cvar_recentMapsBannedNumberMid))
					{
						TrieSetCell( g_recentMapsTrie, recentMapName, 1 );
					}
					else
					{
...
stock configureTheMidPlayersFeature( mapFilerFilePath[] )
{
	...
	if( get_pcvar_num( cvar_voteMidPlayers ) > VOTE_MININUM_PLAYERS_REQUIRED )
	{
		...
		if( mapFilerFilePath[ 0 ] )
		{
			...
			|| mapFilerFilePath[ 0 ] == MAP_CYCLE_LOAD_FLAG )
			{
				...
				TRY_TO_CLEAN( TrieClear, g_midPlayerFillerMapGroupTrie , TrieCreate() )
				...
				loadedCount = processLoadedGroupMapFileFrom( g_midPlayerFillerMapGroupArrays,
				g_voteMidPlayerFillerPathsArray, g_midPlayerFillerMapGroupTrie, false );
...
stock vote_addNominations( blockedMapsBuffer[], &announcementShowedTimes = 0 )
{
	...
	&& ( nominatedMapsCount = ArraySize( g_nominatedMapsArray ) ) )
	{
		...
		if( IS_NOMINATION_MID_PLAYERS_CONTROL_ENABLED() )
		{
			new mapFilerFilePath[ MAX_FILE_PATH_LENGHT ];
			...
			{
				whitelistMapTrie = g_midPlayerFillerMapGroupTrie;
			}
		}
...
stock loadOnlyNominationVoteChoices()
{
	...
	if( IS_NOMINATION_MININUM_PLAYERS_CONTROL_ENABLED()
	|| IS_NOMINATION_MID_PLAYERS_CONTROL_ENABLED()
...
stock map_isTooRecent( map[] )
{
	...
	if( g_recentMapsTrie )
	{
		...
		if(TrieKeyExists(g_recentMapsTrie, map))
		{
			if(IS_NOMINATION_MININUM_PLAYERS_CONTROL_ENABLED())
			{
				new value;
				TrieGetCell(g_recentMapsTrie, map,value);
				return (value>1);				
			}
	...
	else
	{
		...
		if( read_argc() < 3 )
		{
			...
			if( strlen( argument ) > 0 )
			{
				...
				else if(equali( argument, "-now", 4 ) != 1 )
				{
...
stock bool:sayHandlerForOneNomWords( player_id, firstWord[] )
{
	...
	else
	{
		...
		else if( equali(firstWord, "nom")) //strlen( firstWord ) > 5
...
stock nomination_menu( player_id )
{
	...
	for( ; mapIndex < nominationsMapsCount && itemsCount < MAX_NOM_MENU_ITEMS_PER_PAGE; mapIndex++ )
	{
		...
		// 'nomination_menu(1)' and 'nominationAttemptWithNamePart(2)'.
		{
			...
			else if(IS_NOMINATION_MID_PLAYERS_CONTROL_ENABLED()&&!TrieKeyExists( g_midPlayerFillerMapGroupTrie, nominationMap ))
			{
				formatex( disabledReason, charsmax( disabledReason ), "(%d+ players)", get_pcvar_num(cvar_voteMidPlayers));
			}
			else if(IS_NOMINATION_MININUM_PLAYERS_CONTROL_ENABLED()&&!TrieKeyExists( g_minPlayerFillerMapGroupTrie, nominationMap ))
			{
				formatex( disabledReason, charsmax( disabledReason ), "(%d+ players)", get_pcvar_num(cvar_voteMinPlayers));
			}
...
stock map_nominate( player_id, mapIndex )
{
	...
	if( !is_to_block_map_nomination( player_id, mapName ) )
	{
		...
		if( IS_NOMINATION_MID_PLAYERS_CONTROL_ENABLED() )
		{
			if( !TrieKeyExists(g_midPlayerFillerMapGroupTrie,mapName) )
			{
				color_chat( player_id, "!g%s!y cannot be played under !g%d players!y!", mapName, get_pcvar_num(cvar_voteMidPlayers));
				LOG( 1, "    ( map_nominate ) The map: %s, was blocked by the mid map setting.", mapName )
				return;
			}
		}	
		if( IS_NOMINATION_MININUM_PLAYERS_CONTROL_ENABLED() )
		{
			if( !TrieKeyExists(g_minPlayerFillerMapGroupTrie,mapName) )
			{
				color_chat( player_id, "!g%s!y cannot be played under !g%d players!y!", mapName, get_pcvar_num(cvar_voteMinPlayers));
				LOG( 1, "    ( map_nominate ) The map: %s, was blocked by the min map setting.", mapName )
				return;
			}
		}
