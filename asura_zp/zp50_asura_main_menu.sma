...
public plugin_init()
{
	...
	register_clcmd("say zpmenu", "clcmd_zpmenu",0,"- Opens ZP Main Menu")
	register_clcmd("say /premium", "ShowMotd",0,"Premium Info")
	// Menus
	...
	register_menu("Asura Menu", KEYSMENU, "menu_asura")	
}
...
show_menu_main(id)
{
	...
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "\yGamerClub.NeT | Asura Zombie ^n^n", ZP_VERSION_STR_LONG)
	...
	if (LibraryExists(LIBRARY_ZOMBIECLASSES, LibType_Library) && zp_class_zombie_get_count() > 1)
		len += formatex(menu[len], charsmax(menu) - len, "\r3.\w %L^n^n", id, "MENU_ZCLASS")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d3. %L^n^n", id, "MENU_ZCLASS")
		
	// 4. Player Menu
	len += formatex(menu[len], charsmax(menu) - len, "\r4.\w Level Menu^n")

	// 5. FPS
	len += formatex(menu[len], charsmax(menu) - len, "\r5.\w Points Menu^n^n")
	...
	// 7. VIP Menu
	len += formatex(menu[len], charsmax(menu) - len, "\r7.\w VIP Menu^n")

	len += formatex(menu[len], charsmax(menu) - len, "\r8.\w Premium Menu^n^n")
	// 9. Admin menu
	...
	// 7. Asura Menu
	len += formatex(menu[len], charsmax(menu) - len, "\r7.\w Asura VIP Menu^n^n")*/
...
{	
	...
	// 1. VIP Items
	len += formatex(menu[len], charsmax(menu) - len, "\r1.\w VIP Items Menu\y (/vm)^n^n")
		
	// 5. Toggle Zombie VIP Model
	len += formatex(menu[len], charsmax(menu) - len, "\r2.\w Zombie VIP Skin\r [%s]\y (/zskin)^n",((zv_get_user_flags(id)&ZV_MAIN)&&zp_vip_model_get(id))?"ON":"OFF")

	// 6. Toggle VIP Zombie Glow
	len += formatex(menu[len], charsmax(menu) - len, "\r3.\w Zombie VIP Special Glow\r [%s]\y (/zglow)^n^n",((zv_get_user_flags(id)&ZV_MAIN)&&zp_vip_glow_get(id))?"ON":"OFF")
	
	len += formatex(menu[len], charsmax(menu) - len, "\r5.\w VIPs Online\y (/vip)^n^n")
...
public zp_asura_menu_show(id)
{	
	static menu[500]
	new len
	
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "\yGamerClub.NeT | Premium Menu ^n^n", ZP_VERSION_STR_LONG)
	
	// 1. VIP Items
	len += formatex(menu[len], charsmax(menu) - len, "\r1.\w Items Menu\y (/zaitems)^n^n")
	...
	len += formatex(menu[len], charsmax(menu) - len, "\r5.\w Premium Users Online^n^n")

	len += formatex(menu[len], charsmax(menu) - len, "\r7.\w Premium Info^n^n")
	...
	// Fix for AMXX custom menus
	set_pdata_int(id, OFFSET_CSMENUCODE, 0)
	show_menu(id, KEYSMENU, menu, -1, "Asura Menu")
}


// VIP Menu
public menu_asura(id, key)
{
	// Player disconnected?
	if (!is_user_connected(id))
		return PLUGIN_HANDLED;
...
static Motd[1536]
public ShowMotd(id)
	show_motd(id, "prm.txt","Asura Premium Info")
...
public print_adminlist(user) 
{
	...
	len = format(message, 255, "[GC]^3 Premium Users Online:^1 ")
	if(count > 0) {
		for(x = 0 ; x < count ; x++) {
			len += format(message[len], 255-len, "%s^1%s ", adminnames[x], x < (count-1) ? ", ":"")
			if(len > 96 ) {
	...
	{
		len = format(message, charsmax(message), "[GC]^3 No Premium Users Online.")
		ColorChat(user,GREEN,"%s", message)
	}
}
