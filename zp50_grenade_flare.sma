...
new const g_vault_name[] = "gc_flare";
new iHaveF[33]
new g_trailSpr
...
public plugin_init()
{
	...
	register_clcmd("say /flare", "SwapFlare")
...
public plugin_end()
{
	ArrayDestroy(g_sound_grenade_flare_explode)
}
public plugin_natives()
{
	register_native("fps_flare", "SwapFlare", 1)
}
...
SaveExp(plr)
{
    ...
    new data[16];
    num_to_str(iHaveF[plr], data, sizeof(data) - 1);
...
public SwapFlare(id)
{
	if(!iHaveF[id])
	{
		ColorChat(id,GREEN, "[GC]^3 you disabled ^4Flare Light")
		iHaveF[id] = 1
	}
	else
	{
		ColorChat(id,GREEN, "[GC]^3 you enabled ^4Flare Light")
		iHaveF[id] = 0
	}
}
