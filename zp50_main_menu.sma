...
{	
	...
	// 1. Skins Menu
	len += formatex(menu[len], charsmax(menu) - len, "\r1.\w Skins Menu\y (/hclass)^n")
	...
	len += formatex(menu[len], charsmax(menu) - len, "\r4.\w Zombie VIP Skin\r [%s]\y (/zskin)^n",((zv_get_user_flags(id)&ZV_MAIN)&&zp_vip_model_get(id))?"ON":"OFF")

	// 5. Toggle VIP Zombie Glow
	len += formatex(menu[len], charsmax(menu) - len, "\r5.\w Zombie VIP Special Glow\r [%s]\y (/zglow)^n^n",((zv_get_user_flags(id)&ZV_MAIN)&&zp_vip_glow_get(id))?"ON":"OFF")

	// 6. VIP Information
	len += formatex(menu[len], charsmax(menu) - len, "\r6.\w VIP Information\y (/vip)^n^n")
