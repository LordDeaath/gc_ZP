...
{
	if (equal(module, LIBRARY_NEMESIS) || equal(module, LIBRARY_PREDATOR) ||equal(module, LIBRARY_DRAGON) || equal(module, LIBRARY_NIGHTCRAWLER) || equal(module, LIBRARY_ASSASSIN) || equal(module, LIBRARY_SURVIVOR) || equal(module, LIBRARY_SNIPER) || equal(module, LIBRARY_PLASMA) || equal(module, LIBRARY_KNIFER))
...
public client_disconnected(leaving_player)
{
	...
	{
		// Nemesis Left During Plague
		if(zp_class_nemesis_get(leaving_player)&&zp_gamemodes_get_current()==Plague && zp_core_get_zombie_count() > 1)
		{			
			// Find replacement
			while ((id = GetRandomZombie(random_num(1, GetZombieCount()))) == leaving_player ) { /* keep looping */ }
		...
		// Survivor Left During Plague
		else if(zp_class_survivor_get(leaving_player)&&zp_gamemodes_get_current()==Plague&&zp_core_get_human_count()>1)
		{				
			// Find replacement
			while ((id = GetRandomHuman(random_num(1, GetHumanCount()))) == leaving_player ) { /* keep looping */ }
		...
		//Last Zombie
		else if (zp_core_is_zombie(leaving_player) && zp_core_get_zombie_count() == 1)
		{
			...
			else if(LibraryExists(LIBRARY_DRAGON, LibType_Library) && zp_class_dragon_get(leaving_player))
			{
			...
			else if (LibraryExists(LIBRARY_NIGHTCRAWLER, LibType_Library) && zp_class_nightcrawler_get(leaving_player))
			{
		...
		else if (!zp_core_is_zombie(leaving_player) && zp_core_get_human_count() == 1)
		{
			...
			else if (LibraryExists(LIBRARY_KNIFER, LibType_Library) && zp_class_knifer_get(leaving_player))
			{
			...
			else if (LibraryExists(LIBRARY_PLASMA, LibType_Library) && zp_class_plasma_get(leaving_player))
			{
...
// Get Random Zombie -returns index of alive player number target_index -
GetRandomZombie(target_index)
{
	new iAlive, id
...
// Get Random Human -returns index of alive player number target_index -
GetRandomHuman(target_index)
{
	new iAlive, id
