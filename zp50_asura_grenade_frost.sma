...
new cvar_grenade_frost_duration, cvar_grenade_frost_hudicon//, cvar_grenade_frost_frozenhit
...
//	cvar_grenade_frost_frozenhit = register_cvar("zp_grenade_frost_hit", "1")
...
public plugin_end()
{
	ArrayDestroy(g_sound_grenade_frost_explode)
	ArrayDestroy(g_sound_grenade_frost_player)
	ArrayDestroy(g_sound_grenade_frost_break)
}
...
public fw_TraceAttack(victim, attacker)
{
	...
	if(flag_get(g_IsFrozen, attacker) && zp_class_predator_get(victim))
	{
		return HAM_SUPERCEDE;
	}
		
	if (flag_get(g_IsFrozen, attacker) && zp_core_is_zombie(attacker))
		return HAM_SUPERCEDE;

	if(flag_get(g_IsFrozen, victim) && zp_class_dragon_get(victim))
		return HAM_SUPERCEDE;	
		
	if(flag_get(g_IsFrozen, victim) && zp_class_nemesis_get(victim))
		return HAM_SUPERCEDE;
		
	//if(flag_get(g_IsFrozen, victim) && zp_core_is_zombie(victim))
...
frost_explode(ent)
{
	...
	/*new AuthID[32]
	new isEmma,isNala;
	new NadeOwner = pev(ent, pev_owner)
	get_user_authid(NadeOwner ,AuthID,31)
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, origin, NADE_EXPLOSION_RADIUS)) != 0)
	{
		...
		/*if(isEmma||isNala)
		{
			...
			if(equali(AuthID,"STEAM_0:0:176769118"))
			{	
				CapCount++;
				ColorChat(victim,GREEN,"[DS] The Ladies^3 have frozen you ^4%d times^3 this map! Lame...",CapCount)					
				ColorChat(NadeOwner,GREEN,"[DS] YOU^3 and ^4YOUR SISTER/FRIEND^3 have frozen ^4zXCaptainXz %d times^3 this map! Lame...",CapCount)
				new Sister;
				if(isEmma)
				{	
					Sister=find_player("c","STEAM_0:0:2055819299")
					if(is_user_connected(Sister))
					{
						ColorChat(Sister,GREEN,"[DS] YOU^3 and ^4YOUR SISTER/FRIEND^3 have frozen ^4zXCaptainXz %d times^3 this map! Lame...",CapCount)
					}
					isEmma=false;
				}
				else
				if(isNala)
				{
					Sister=find_player("c","STEAM_0:0:555283776")
					if(is_user_connected(Sister))
					{
						ColorChat(Sister,GREEN,"[DS] YOU^3 and ^4YOUR SISTER/FRIEND^3 have frozen ^4zXCaptainXz^3 %d times this map! Lame...",CapCount)
					}
					isNala=false;
				}
			}			
...
public remove_freeze(taskid)
{
	...
	//if(!zp_is_blinded(ID_FROST_REMOVE))
	//{
