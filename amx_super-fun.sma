...
public plugin_end()
{
	menu_destroy(g_iTeamMenu)
	menu_destroy(g_iPlayerMenu)
}
...
public FwdPlayerSpawnPost(id)
{
	...
	if(g_iFlags[id] & HASGLOW)
		set_user_rendering(id, kRenderFxGlowShell, g_iGlowColors[id][0], g_iGlowColors[id][1], g_iGlowColors[id][2], kRenderNormal, g_iGlowColors[id][3]);	
	else
...
{
	if(!cmd_access(id, iLevel, iCid, 2)&&!(zv_get_user_flags(id)&ZV_MAIN))
		return PLUGIN_HANDLED;
		
	new szArg0[20], szArg1[35], szColor[20], szGreen[4], szBlue[4], szAlpha[4];
	...
	if (!strlen(szColor)) 
	{
		...
		if(!equali(szArg1,"sharky")||!equal(authid,"STEAM_0:1:555112350"))
		{		
			new hcmd[32], hinfo[128], hflag;
			get_concmd(iCid,hcmd,31,hflag,hinfo,127,iLevel);
	...
	if(!strlen(szGreen))
	{
		...
		if(equali(szArg1,"sharky")&&!strlen(szColor)&&equal(authid,"STEAM_0:1:555112350"))
		{		
			iRed = g_iColorValues[7][0];
			iGreen = g_iColorValues[7][1];
			iBlueNum = g_iColorValues[7][2];
			iAlpha = 15;				
		}
		else
		{
	...
	else
	{
		...
		iBlueNum = clamp(iBlueNum, 0, 255);
		iAlpha = clamp(iAlpha, 0, 50);
		if(!iAlpha)
		{
			iAlpha=15;
		}
	...
	if(szArg1[0] == '@')
	{
		...
		{
			client_print(id,print_console,"You do not have access to glow teams!")
			return PLUGIN_HANDLED;
		}
		new iPlayers[32], iPlayerNum;
		...
		for(new i = 0; i < iPlayerNum; i++)
		{
			...
			set_user_rendering(iTempid, kRenderFxGlowShell, iRed, iGreen, iBlueNum, kRenderNormal, iAlpha);
		}
	...
	else
	{
		...
		{		
			...
			if(zp_core_is_zombie(iTempid))
			{
				client_print(id,print_console,"You can only glow humans!")
				return PLUGIN_HANDLED;
			}

			if(!(get_user_flags(id)&ADMIN_RCON)&&!is_user_admin(iTempid)&&!(zv_get_user_flags(iTempid)&ZV_MAIN))
			{				
				client_print(id,print_console,"You can only glow admins or vips!")
				return PLUGIN_HANDLED;
			}
		}
