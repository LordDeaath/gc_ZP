...
// Buy Menu: Admin Weapons
new const admin_items[][] = { "weapon_sg550" , "weapon_g3sg1" , "weapon_galil" , "weapon_famas" , "weapon_m4a1" , "weapon_ak47" , "weapon_sg552" , "weapon_aug" , "weapon_m249", "weapon_xm1014", "weapon_awp", "weapon_scout" , "weapon_m3" , "weapon_tmp" , "weapon_mac10" , "weapon_ump45" , "weapon_mp5navy" , "weapon_p90"
}
...
new const WEAPONNAMES[][] ={ "", "P228", "", "Scout", "HE Grenade", "XM1014", "", "MAC-10", "AUG",
"Smoke Grenade", "Dual Elite", "FiveSeven", "UMP", "SG-550", "Galil", "Famas",
...
new g_CanBuyPrimary
new g_CanBuyPrimary2
new g_CanBuySecondary
new g_CanBuyGrenades
new g_MenuData[MAXPLAYERS+1][6]
...
public plugin_init()
{
	...
	register_clcmd("say /buy", "clcmd_buy",0,"- Opens ZP Weapon Menu")
	register_clcmd("say buy", "clcmd_buy",0,"- Opens ZP Weapon Menu")
	register_clcmd("say /guns", "clcmd_buy",0,"- Opens ZP Weapon Menu")
	register_clcmd("say guns", "clcmd_buy",0,"-Opens ZP Weapon Menu")
	
	// Menus
	register_menu("Buy Menu Primary", KEYSMENU, "menu_buy_primary")	
	register_menu("Buy Menu Admin Primary", KEYSMENU, "menu_buy_primary2")
...
public plugin_end()
{	
	ArrayDestroy(g_primary_items)
	ArrayDestroy(g_secondary_items)
	ArrayDestroy(g_grenades_items)
}
...
{
	if (equal(module, LIBRARY_SURVIVOR) || equal(module, LIBRARY_SNIPER) || equal(module, LIBRARY_KNIFER) || equal(module, LIBRARY_PLASMA))
...
{
	if(zp_gamemodes_get_current()==Tag||zp_gamemodes_get_current()==Wars||zp_gamemodes_get_current()==GG)
		return false;
...
public human_weapons(id)
{
	...
	// Survivor,Sniper,Knifer Plasma and NInja automatically gets his own weapon
	if (LibraryExists(LIBRARY_SURVIVOR, LibType_Library) && zp_class_survivor_get(id) || LibraryExists(LIBRARY_SNIPER, LibType_Library) && zp_class_sniper_get(id) || LibraryExists(LIBRARY_PLASMA, LibType_Library) && zp_class_plasma_get(id) || LibraryExists(LIBRARY_KNIFER, LibType_Library) && zp_class_knifer_get(id))
	{
		flag_unset(g_CanBuyPrimary, id)		
		flag_unset(g_CanBuyPrimary2, id)
	...
	if (get_pcvar_num(cvar_buy_custom_primary))
	{
		...
		if (is_user_bot(id))
			buy_primary_weapon(id, random_num(0, ArraySize(g_primary_items) - 1))
		else if (!WPN_AUTO_OFF&&Selected[id])
			buy_primary_weapon(id, WPN_AUTO_PRI)
	}
	if(get_user_flags(id)&ADMIN_KICK)
	{		
		flag_set(g_CanBuyPrimary2, id)

		if (!WPN_AUTO_OFF&&Selected[id])
			buy_primary_weapon2(id, WPN_AUTO_PRI2)
	}
...
show_available_buy_menus(id)
{
	...
	else if (flag_get(g_CanBuyPrimary2, id))
		show_menu_buy_primary2(id)
...
show_menu_buy_primary(id)
{
	...
	// 8. Auto Select
	len += formatex(menu[len], charsmax(menu) - len, "^n\r8.\w %L \y[%L]", id, "MENU_AUTOSELECT", id, (WPN_AUTO_OFF) ? "MOTD_DISABLED" : "MOTD_ENABLED")
...
show_menu_buy_primary2(id)
{
	...
	static menu[300]
	new len, keys, index, maxloops = min(WPN_STARTID+7, WPN_MAXIDS2)
	keys = KEYSMENU
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "\rADMIN\y Primary Weapons \r[%d-%d]^n^n",  WPN_STARTID+1, min(WPN_STARTID+7, WPN_MAXIDS2))
	
	// 1-7. Weapon List
	for (index = WPN_STARTID; index < maxloops; index++)
	{
		if(user_has_weapon(id, get_weaponid(admin_items[index])))
		{
			len += formatex(menu[len], charsmax(menu) - len, "\d%d. %s^n", index-WPN_STARTID+1, WEAPONNAMES[get_weaponid(admin_items[index])])
			keys = KEYSMENU &~(1<<(index-WPN_STARTID))
			continue;
		}		
		len += formatex(menu[len], charsmax(menu) - len, "\r%d.\w %s^n", index-WPN_STARTID+1, WEAPONNAMES[get_weaponid(admin_items[index])])
	}
	...
	// Fix for AMXX custom menus
	set_pdata_int(id, OFFSET_CSMENUCODE, 0)
	show_menu(id, keys, menu, menu_time, "Buy Menu Admin Primary")
}
...
show_menu_buy_secondary(id)
{
	...
	// 8. Auto Select
	len += formatex(menu[len], charsmax(menu) - len, "^n^n\r8.\w %L \y[%L]", id, "MENU_AUTOSELECT", id, (WPN_AUTO_OFF) ? "MOTD_DISABLED" : "MOTD_ENABLED")
...
public menu_buy_primary(id, key)
{
	...
	if (key >= MENU_KEY_AUTOSELECT || WPN_SELECTION >= WPN_MAXIDS2)
	{
		switch (key)
		{
			...
			{
				WPN_AUTO_OFF = 1 - WPN_AUTO_OFF
			}
			case MENU_KEY_NEXT: // next/back
			{
				if (WPN_STARTID+7 < WPN_MAXIDS2)
					WPN_STARTID += 7
				else
					WPN_STARTID = 0
			}
			...
			{
				if(zp_class_zombie_get_next(id)==ZP_INVALID_ZOMBIE_CLASS)
				{
...
// Buy Menu Primary2
public menu_buy_primary2(id, key)
{
	if(zp_gamemodes_get_current()==Tag||zp_gamemodes_get_current()==Wars||zp_gamemodes_get_current()==GG)
		return PLUGIN_HANDLED;

	// Player dead or zombie or already bought primary2
	if (!is_user_alive(id) || zp_core_is_zombie(id) || !flag_get(g_CanBuyPrimary2, id))
		return PLUGIN_HANDLED;
...
// Buy Primary Weapon
buy_primary_weapon2(id, selection)
{
	if(zp_gamemodes_get_current()==Tag||zp_gamemodes_get_current()==Wars||zp_gamemodes_get_current()==GG)
		return;
	
	new weaponid = get_weaponid(admin_items[selection])
	
	// Give the new weapon and full ammo
	give_item(id, admin_items[selection])
	ExecuteHamB(Ham_GiveAmmo, id, MAXBPAMMO[weaponid], AMMOTYPE[weaponid], MAXBPAMMO[weaponid])
