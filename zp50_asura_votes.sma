...
{
	if(!(get_user_flags(id) & ADMIN_KICK)&&!(zv_get_user_flags(id)&ZV_DAMAGE))
	{
		ColorChat(id, GREEN, "[GC]^3 Special Round Votes^1 can be only started by^3 Admins/Asura VIP^1 and above")
		client_print(id, print_console, "Special Round Votes can be only started by Admins/Asura VIP and above")
